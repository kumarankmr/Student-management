...
@login_required(login_url='login')  # Replace 'login' with your login URL name if needed
def afterlogin_view(request):
    # Check if the user is a staff member
    if request.user.is_staff:
        return redirect('staff_dashboard')  # Redirect to staff dashboard
    # Check if the user is a student (make sure you have a proper way of identifying this)
    elif hasattr(request.user, 'is_student') and request.user.is_student:
        return redirect('student_dashboard')  # Redirect to student dashboard
    # If neither staff nor student, return an error or handle as needed
    else:
        return render(request, 'school/afterlogin.html')  # Ensure this returns a response
